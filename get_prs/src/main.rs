...
async fn prs(week_spec: &str) -> Result<u64, Box<dyn Error>> {
    ...
    let spec = " is:pr org:graphql is:merged merged:".to_owned() + week_spec;
    let token = token()?;
    ...
    for pr in prs {
        let url = pr.html_url;
        if let Some(unprefixed) = url.path().strip_prefix("/graphql/") {
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match cmd.as_ref().map_or("", |s| s) {
        "week" => {
            ...
            println!(
                "https://github.com/search?q=is%3Apr+org%3Agraphql+is%3Amerged+merged%3A{}",
                week_spec
            );
        }
        "token" => {
        ...
        "branches" => {
            let branches_out = command("git", &["branch"], TWiG)?;
            let mut current = "";
...
\n[merged]: https://github.com/search?q=is%3Apr+org%3Agraphql+is%3Amerged+merged%3A{week_spec}\n\
\n{filtered_prs}"
            );
            ...
            for chapter in chapters {
                ...
                match title.trim() {
                    ...
                    "Updates from the GraphQL Projects" => {
                        ...
                        assert!(link.starts_with("[merged]: https://github.com/search?q=is%3Apr+org%3Agraphql+is%3Amerged+merged%3A"));
        ...
        "push" => {
            ...
            command("git", &["commit", "-m", "C/QotW and notable changes"], TWiG)?;
            command("git", &["push", "doc-jones"], TWiG)?;
            // open the PR view
            command(
                firefox,
                &[
                    "--new-tab",
                    &format!(
                        "https://github.com/doc-jones/this-week-in-graphql/pull/new/twig-{}",
                        number
                    ),
                ],
                "",
            ...
            if let Ok(num) = str::parse::<u64>(number) {
                ...
                command("git", &["branch", "-d", &previous_branch], TWiG)?;
                command(
                    "git",
                    &["push", "--delete", "doc-jones", &previous_branch],
                    TWiG,
                )?;
            }
        }
        _ => {
            println!("usage: TWIG_RUST [prs <spec>|filter]");
        }
    }
    Ok(())
}
...
fn check_link(link: &str) -> usize {
    if let Some(rest) = link.strip_prefix("https://github.com/graphql/") {
